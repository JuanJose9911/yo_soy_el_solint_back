                ...
                <table class="w-100 jojo ">
                    ...
                    @foreach ($fees as $fee)
                        <tr style=" padding-top:-1px">
                            ...
                            <td class="td-center">${{ number_format($fee['total_due_paid']) }}</td>
                            @php ($total_due += $fee['total_due_paid'])
                            <td class="td-center">${{ number_format($fee['total_interest_paid']) }}</td>
                            @php ($total_interest += $fee['total_interest_paid'])
                            <td class="td-center">${{ number_format($fee['total_late_paid']) }}</td>
                            @php ($total_late += $fee['total_late_paid'])
                            <td class="td-center">${{ number_format(intval($fee['total_due_paid'] + $fee['total_interest_paid'] + $fee['total_late_paid'])) }}</td>
                            @php ($total = $total + $fee['total_due_paid'] + $fee['total_interest_paid'] + $fee['total_late_paid'])
                        </tr>
                    @endforeach
                    <tr class="tr-bottom">
                        <td class="td-center"><strong>Aplicado</strong></td>
                        <td class="td-center"><strong>${{ number_format(intval($total_due)) }}</strong></td>
                        <td class="td-center"><strong>${{ number_format(intval($total_interest)) }}</strong></td>
                        <td class="td-center"><strong>${{ number_format(intval($total_late)) }}</strong></td>
                        <td class="td-center"><strong>${{ number_format(intval($total)) }}</strong></td>
                    </tr>
                    </tbody>
                </table>
            </div>
